...
### D-015 · 前端框架：Vue3 vs React

**结论**：采用 Vue3 + TypeScript + Pinia + Tailwind CSS。

**理由**：用户明确要求 Vue3，与主站（纯静态 HTML）保持独立，后续通过构建产物集成。

---

### D-016 · 数据存储：localStorage vs 后端数据库

**结论**：全部数据存储在浏览器 localStorage，后端仅提供解析和 AI 优化 API。

**理由**：
- 用户要求除解析和 AI 外数据全在本地；
- 减轻后端复杂度，无需数据库；
- 主站已有 localStorage 使用模式，保持一致；
- 局限：数据仅在单浏览器可用，不可跨设备同步。

---

### D-017 · 简历解析范围：仅经历与自我描述

**结论**：后端仅从 PDF/DOCX 中解析提取项目/实习经历和自我描述，基本信息由用户手动填写。

**理由**：
- 基本信息字段结构化程度高，手动填写更准确；
- 解析聚焦于非结构化文本（经历段落、自我评价），减少误提取；
- 正则拆分失败时使用 DeepSeek AI 兜底。

---

### D-018 · PDF 导出方案：前端 html2canvas + jsPDF

**结论**：在前端使用 html2canvas + jsPDF 生成 PDF，不经过后端。

**理由**：
- 前端可完全控制排版样式，实现多模板切换；
- 无需后端渲染服务（如 wkhtmltopdf）；
- 局限：复杂排版（如分页断点）需手动调优。
...
**设计要素**：
- 配色：米黄纸张 #fdf6e3、深棕文字 #3e2723、手写蓝 #5c6bc0、橙色点缀 #ff8a65
- 字体：ZCOOL KuaiLe（手写中文）、Caveat（手写英文）
- 元素：手绘边框（不规则 border-radius）、便签卡片、笔记本横线背景
- PDF 模板：校园手绘风 + 简洁表格两种可选
